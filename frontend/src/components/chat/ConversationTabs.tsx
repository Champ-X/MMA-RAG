...
export function ConversationTabs({
  ...
  return (
    <div className="relative flex-shrink-0 border-b border-violet-300/60 bg-gradient-to-br from-violet-200/95 via-indigo-100 to-violet-100/90 shadow-[0_1px_0_rgba(255,255,255,0.55)_inset] backdrop-blur-md dark:border-indigo-800/50 dark:from-indigo-950 dark:via-violet-950/80 dark:to-indigo-950 dark:shadow-[0_1px_0_rgba(255,255,255,0.04)_inset]">
      ...
      <div
        ref={scrollRef}
        className="flex min-h-0 items-center gap-2.5 overflow-x-auto px-4 py-1.5 scrollbar-hide"
      >
        {conversations.map((conv) => {
          ...
          return (
            <div
              ...
              className={cn(
                'group relative flex min-w-0 max-w-[min(280px,72vw)] shrink-0 items-center gap-0.5 overflow-hidden rounded-full py-2 pl-3.5 pr-1 transition-all duration-200 ease-out',
                isActive
                  ? 'bg-gradient-to-r from-indigo-400 to-violet-500 text-white shadow-sm shadow-indigo-500/15 ring[2.3px] ring-white/35 dark:from-indigo-400/95 dark:to-violet-500/95 dark:shadow-indigo-950/40 dark:ring-white/22'
                  : 'bg-violet-50/95 text-indigo-950 ring-[2.3px] ring-indigo-400/55 shadow-sm shadow-indigo-500/10 hover:bg-indigo-50 hover:text-indigo-950 hover:ring-indigo-500/65 hover:shadow-md hover:shadow-indigo-500/15 dark:bg-violet-950/55 dark:text-violet-50 dark:ring-violet-400/50 dark:shadow-md dark:shadow-violet-950/30 dark:hover:bg-violet-900/65 dark:hover:text-white dark:hover:ring-violet-300/45'
              )}
            >
              {isActive && (
                <span
              ...
              <button
                type="button"
                className={cn(
                  'relative z-[1] min-w-0 flex-1 truncate text-left text-[15px] font-medium leading-tight tracking-tight',
              ...
              {conversations.length > 1 && (
                <button
                  ...
                  title="删除对话"
                  className={cn(
                    'relative z-[1] shrink-0 rounded-full p-0.5 transition-all duration-200',
                    isActive
        ...
        <button
          ...
          onClick={onCreate}
        >
          <Plus size={19} strokeWidth={2.5} className="shrink-0" />
        </button>
      </div>
    </div>
  )
