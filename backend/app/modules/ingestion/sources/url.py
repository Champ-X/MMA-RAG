...
INSPECT_MAX_BYTES = 256 * 1024  # 轻量预判时最多读取 256KB
...
def _extract_html_metadata(html_bytes: bytes, url: str) -> tuple[Optional[str], Optional[str]]:
    title: Optional[str] = None
    site: Optional[str] = None
    try:
        ...
        def _meta(*attrs: tuple[str, str]) -> Optional[str]:
        ...
        title = (
            ...
            or ((soup.title.get_text(" ", strip=True)[:300]) if soup.title and soup.title.get_text(" ", strip=True) else None)
        )
        site = _meta(("property", "og:site_name")) or (urlparse(url).hostname or None)
    except Exception:
        text = html_bytes.decode("utf-8", errors="ignore")
        title_match = re.search(r"<title[^>]*>(.*?)</title>", text, re.I | re.S)
        if title_match:
            title = re.sub(r"\s+", " ", title_match.group(1)).strip()[:300] or None
        ...
        if site_match:
            site = re.sub(r"\s+", " ", site_match.group(1)).strip()[:300] or None
    return title, site or (urlparse(url).hostname or None)
...
def _build_webpage_result(
    ...
    if ext.get("structured_strategy"):
        cleaned["structured_strategy"] = ext["structured_strategy"]
...
class UrlSource(BaseContentSource):
    ...
    async def inspect_async(
        ...
        *,
        mode: UrlMode = "auto",
        max_inspect_bytes: int = INSPECT_MAX_BYTES,
    ) -> UrlInspectionResult:
        ...
        async with httpx.AsyncClient(
            ...
            async with client.stream("GET", url) as response:
                ...
                async for chunk in response.aiter_bytes():
                    if not chunk:
                        continue
                    remaining = max_inspect_bytes - sampled_bytes
                    if remaining <= 0:
                        break
                    piece = chunk[:remaining]
                    sampled.append(piece)
                    sampled_bytes += len(piece)
                    if sampled_bytes >= max_inspect_bytes:
                        break

                sample = b"".join(sampled)
                ...
                site: Optional[str] = None
                if is_html:
                    title, site = _extract_html_metadata(sample, final_url)
