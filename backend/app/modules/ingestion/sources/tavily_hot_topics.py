...
def fetch_hot_topics(
    ...
    t = topic or getattr(settings, "tavily_hot_topics_topic", "news")
    tr = time_range or getattr(settings, "tavily_hot_topics_time_range", "day")
    mr = max_results if max_results is not None else getattr(settings, "tavily_max_results", 10)
    sd = search_depth or getattr(settings, "tavily_search_depth", "basic")
