        ...
        try:
            ...
            for result in retrieval_result.reranked_results:
                ...
                if content_type == "image":
                    ...
                    # 合并检索返回的 metadata（含内部关键视觉证据），供 format_video_content 使用。
            ...
            for ref_num in unique_ref_nums:
                if ref_num in reference_map:
                    ...
                    if isinstance(ref_id, str) and ref_id.isdigit():
                        ref_id = int(ref_id)
                    # 关键帧属于检索/生成的内部视觉证据，不能透传到回答的公开引用载荷；
                    # 否则任一客户端都可能把它们当作普通图片批量渲染。
                    public_metadata = {
                        key: value
                        for key, value in (reference.metadata or {}).items()
                        if key not in {"key_frames", "keyframes"}
                    }
                    ref_dict = {
                        "id": ref_id,
