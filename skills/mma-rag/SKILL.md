---
name: mma-rag
description: Operate the locally running MMA-RAG knowledge service through its bundled deterministic CLI. Use when Codex needs to list or create MMA-RAG knowledge bases, inspect indexed files, upload and index local documents/images/audio/video, wait for ingestion, retrieve compact multimodal evidence, or answer questions grounded in MMA-RAG.
---

# MMA-RAG

Use the bundled `scripts/mma-rag` CLI. Resolve it relative to this `SKILL.md` and invoke it by absolute path. Expect JSON on stdout for every command.

## Workflow

1. Run `scripts/mma-rag health` before the first operation.
2. If the service is unavailable, report that MMA-RAG must be started. Do not start or stop it unless the user asks.
...
8. Use `ask` when the user explicitly wants MMA-RAG to generate the grounded answer.
...
- Never delete or overwrite MMA-RAG content; this skill exposes no deletion command.
