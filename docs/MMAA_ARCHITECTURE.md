...
- **多模态数据**：支持文档（PDF、Word、TXT、Markdown 等）、图片、以及扩展中的音频/视频；文档与图片已全流程打通，音频/视频在意图与流水线上有预留与部分实现。
- **语义路由与混合检索**：根据查询自动路由至合适知识库；混合检索结合 Dense（语义向量）、Sparse（BGE-M3 稀疏向量）、Visual（CLIP + VLM 描述）三路，并两阶段重排（RRF 粗排 + Cross-Encoder 精排）。
- **统一 LLM 管理层**：意图识别、VLM 描述、Embedding、Reranker、最终生成等由模块化 LLM Manager 统一调度，支持多厂商 API（如 SiliconFlow、OpenRouter、阿里云百炼、DeepSeek 等）。
...
- **解析入口**：根据文件扩展名或内容检测由 ParserFactory 选择对应 Parser；PDF/DOCX/TXT/Markdown 等返回统一结构的 parse_result（如 markdown 文本、提取的图片列表、元数据），图片解析输出 base64 与尺寸等供 VLM/CLIP 使用。
...
- **多来源接入**：sources 层（URL、文件夹、Tavily 热点、媒体下载等）产出统一格式的「待处理文件」或 URL，由 Ingestion 统一执行下载（若需要）、解析、分块、向量化、写入；大任务通过 Celery 异步执行，进度写入 Redis 供前端轮询或 SSE 推送。

### 2.2 分块策略

- **递归语义分块**：按段落/句子与长度限制递归切分，保留语义边界。
- **重叠窗口**：无清晰结构时使用固定大小 + 重叠（如 max_chunk_size=1000，chunk_overlap=200）。
- 配置参数集中管理，便于调优。

**实现方案要点：**

- 分块入口根据解析结果类型（Markdown、纯文本等）选择策略：已有结构的按标题/段落先切大块，单块若仍超过 `max_chunk_size`（如 1000 字符）则进入递归切分；递归时优先按句号、换行等边界再切，并遵守 `min_chunk_size`（如 100 字符）避免过碎。
- 重叠在递归完成后统一施加：对相邻 chunk 在边界处取 `chunk_overlap`（如 200 字符）的公共内容，保证上下文连贯、检索时边界信息不丢失。
- 文档内嵌图片的处理顺序为：先对文档解析得到的图片逐张做 VLM 描述并上传 MinIO，再将每张图的 caption 按占位符插回 Markdown 原文（如 `[图注：xxx]`），最后对这份「补全后的 Markdown」做上述分块与向量化，这样同一段文字中若含图片说明，会与周围文本一起被切进同一或相邻 chunk，便于检索与引用。
- 每个 chunk 写入向量库时携带 `context_window`：保存前一个与后一个 chunk 的 ID（或临时 ID），便于调试时拉取「前文/后文」做上下文透视（Small-to-Big 扩展可按需使用）。
...
- **多模态统一**：Text Collection 与 Image Collection 的文本侧使用同一 Embedding 模型，便于知识库画像采样时把「文档 chunk」与「图片描述」的向量放在同一空间做聚类。

### 2.4 存储架构

#### 对象存储（MinIO）

- 按知识库与类型组织；路径形式如 `kb_id/...`，文档与图片等分目录存放。
- 上传、解析后写入 MinIO；对外提供 Presigned URL 供前端预览与引用。
...
- **kb_portraits**：知识库画像；向量为 4096 维；Payload 含 kb_id、topic_summary、cluster_size，用于路由阶段相似度检索与加权打分。
...
- **MinIO**：按知识库划分存储空间（如按 kb_id 的 bucket 或前缀），文档与图片等可分子目录；对象路径与 `file_id`、`file_path` 在向量库 Payload 中一致保存，便于生成 Presigned URL 与删除时联动。
- **Qdrant**：所有检索均先按 `kb_id` 做 Pre-filter，再执行向量/稀疏检索，保证只命中目标知识库。`text_chunks` 的 Payload 中 `context_window` 存前后 chunk 的 ID，便于调试接口按 chunk_id 拉取前后文。
- **画像更新触发**：在内容增量或定时策略下触发画像重建；画像生成完成后采用 Replace 策略：先删除该 kb_id 下全部旧画像点，再插入新生成的 portrait 点，避免历史画像残留。
...
- **visual_intent**：explicit_demand / implicit_enrichment / unnecessary，用于决定是否执行 Visual 检索及权重。
...
- **画像生成**（`knowledge/portraits.py`）：
  - 从 Text Collection 与 Image Collection（text_vec）按比例采样向量，K-Means 聚类（K 由轮廓系数/肘部法则等确定）。
  - 对每个簇取近中心若干 Chunk，经 LLM 生成 topic_summary，再向量化写入 `kb_portraits`；采用 Replace 策略更新该 KB 的画像。
...
- **画像生成**：从该 KB 的 Text Collection 与 Image Collection（仅用 text_vec）中按比例采样向量（如文本与图片各占一半）；若总 chunk 数小于阈值（如 5000）则全量取，否则蓄水池采样并设上下限（如 50～1000）。采样时只带 id、vector、source_type（doc/image/audio），不加载正文以节省内存；确定聚类中心后，再按 id 回查各 Collection 取正文。K-Means 的 K 取 `sqrt(N/2)` 并限制在配置的 `max_kb_portrait_size` 内。每个簇取距离中心最近的 5～10 个样本，将其文本以「[文档片段]」「[图片描述]」等前缀拼成 content_pieces，调用 LLM 生成一条 topic_summary，再对该摘要做 Dense 向量化；将该向量与 kb_id、topic_summary、cluster_size 写入 `kb_portraits`。存储前先删除该 kb_id 下全部旧画像（Replace 策略）。
...
- **输入**：RetrievalContext（含 refined_query、target_kb_ids、search_strategies、visual_intent 等）。
- **文本流**：
  - **Dense**：主查询 dense_query + 多视角 multi_view_queries 向量化后检索，内部加权融合。
...
- **图片流**（当 visual_intent 非 unnecessary）：
  - 查询的文本向量匹配 text_vec；CLIP 文本向量匹配 clip_vec；Qdrant Prefetch + Fusion RRF 双路融合。
- **融合**：Dense、Sparse、Visual 三路结果经加权 RRF 粗排（权重可配置），再经 Cross-Encoder 精排取 Top-K，供上下文构建使用。
...
- **RRF**：三路（及可选音频、视频路）结果按 doc id 去重合并后，对每个文档的「多路排名」应用加权 RRF 公式（如 score = Σ weight_t / (k + rank_t)），权重可配（如 dense=1.0、sparse=0.8、visual=1.2），k 通常取 60。RRF 后得到粗排列表，进入精排阶段。

### 3.4 两阶段重排

- **粗排**：RRF 将多路检索结果归一化到排名空间，避免分数量纲不一致；visual 权重可根据 visual_intent 调节（如 explicit 时提高），必要时可对图片结果做配额保证。
- **精排**：Reranker（如 Qwen3-Reranker-8B / BGE-Reranker-v2-m3）对 (Query, Content) 对打分，按分数取最终 Top-K；Content 对文档为 text_content，对图片为 caption。

**实现方案要点：**

- **粗排**：在 HybridSearchEngine 内完成，输出为带 RRF 分数的合并列表；可对 visual_intent 为 explicit 时提高 visual 权重，或对 implicit 时为图片预留最低配额，避免图片被文本结果完全挤掉。
- **精排**：从粗排结果中取前若干名（如 20）作为候选，构建 (query, content) 对：文档用 text_content，图片用 caption（可加「[图片描述]:」前缀）。调用 Cross-Encoder 批量打分后，将精排分数与 RRF 分数按权重（如 0.7 精排 + 0.3 RRF）合并，再按合并分数排序，取 final_top_k（如 10）条。
- **图片保护**：当 visual_intent 为 implicit_enrichment 时，最终截取 Top-K 时可采用「至少保留若干条图片」的策略（如先取精排后的 Top-K，若图片数不足则从剩余候选中补足图片条数），保证隐性需求下仍有一定图片曝光。
...
- 接收重排后的 Top-K（文本 Chunk + 图片等），建立**会话级引用映射**：序号 1,2,3... 对应 chunk_id、file_path、MinIO URL 等。
...
- 系统提示词（`core/llm/prompt.py`、`generation/templates/system_prompts.py`）规定引用格式（如 `[id]`）、多模态描述方式及诚实回答原则。

**实现方案要点：**

- **引用映射**：对重排后的结果按最终分数排序，依次分配序号 1、2、3…；每条结果对应一个 ReferenceMap 条目，包含 id（序号）、content_type（doc/image/audio/video）、file_path、content（摘要或 caption）、metadata（含 score、chunk_id、kb_id 等）、以及可选的 presigned_url。文档类引用保留 chunk_id，便于调试时按 point id 拉取 context_window 前后文。
- **上下文长度控制**：设 max_context_length（如 4000 字符）、max_chunks、max_images（及 implicit 时略多的 max_images_implicit），按相关性顺序填入模板直至达限；超长时可在不破坏引用序号的前提下截断或省略部分材料，并保证 reference_map 与 prompt 中的序号一致。
- **多模态与扩展**：音频、视频引用除文本描述外，可生成 Presigned URL 供前端播放；视频可带关键帧信息或关键帧图片 URL，并将关键帧作为独立「图片」引用加入 reference_map，使模型能以 [n] 引用关键帧图。
- **系统提示词**：按意图类型（如 factual / analysis / creative）可选用不同 system prompt 模板；模板中明确要求回答中事实必须带 [id]、禁止捏造编号、多模态材料需在文中点明类型，以及「未找到则诚实说明」等原则。
...
- **SSE 事件类型**：`thought` 用于推送意图识别、路由、检索策略等中间状态，前端据此更新 ThinkingCapsule（如 intent_type、refined_query、target_kbs、visual_intent、sparse_keywords 等）。`citation` 在上下文构建完成后、生成开始前发送，payload 为 reference_map 的 frontend 友好格式（id、type、file_name、content 摘要、img_url/audio_url/video_url、scores、debug_info 含 chunk_id 与 context_window）；前端可预加载引用卡片，并在用户悬停 [n] 时展示。`message` 为 LLM 流式输出的 delta，前端做打字机渲染与 Markdown 解析。
- **引用与调试**：doc 类型引用在 debug_info 中带 chunk_id；若后端支持，可异步拉取该 chunk 的 context_window（prev/next 文本）并注入 citation，供「上下文透视」类调试组件使用。

---

## 五、模块化 LLM 管理器

- **职责**：统一封装 chat、embed、vision、rerank 等能力，业务层按任务类型调用，不关心具体厂商或模型实例。
- **核心位置**：`backend/app/core/llm/`。
  - **manager**：对外接口与任务路由。
  - **registry**：模型注册表与任务到模型的映射（如 intent_recognition、image_captioning、final_generation、reranking）。
  - **prompt.py**：所有提示词模板字符串；**prompt_engine.py**：加载 prompt 模块并对外提供 `render_template` 等。
  - **providers**：silicon_flow、openrouter、aliyun_bailian、deepseek 等，实现统一协议（如 OpenAI 兼容）。
- **其他核心组件**：
  - **sparse_encoder.py**：BGE-M3 稀疏编码，供 Ingestion 写入与 Retrieval 查询使用。
  - **portrait_trigger.py**：知识库画像更新触发逻辑。
...
- **任务路由**：业务层调用时指定 `task_type`（如 intent_recognition、image_captioning、final_generation、reranking、kb_portrait_generation）；LLM Manager 根据 registry 将任务映射到具体模型与 provider，同一任务可配置主备模型，便于后续做故障转移。
- **统一接口**：chat（含多轮消息、temperature）、embed（文本列表）、rerank（query + documents）等对上层统一；底层各 provider 实现 OpenAI 兼容的请求/响应格式，Manager 负责拼装与解析。
- **提示词**：所有模板字符串集中在 `prompt.py`，由 `prompt_engine` 加载并对外提供 `render_template(template_name, **kwargs)`；各业务模块只传变量名与值，不写死 Prompt 内容，便于迭代与多语言扩展。
...
- **动态思维胶囊**：展示 intent_type、refined_query、路由结果（目标 KB、全库 fallback）、检索策略（Dense/Sparse/Visual）。
- **多模态流式渲染**：SSE 解析 thought / citation / message；引用处悬浮预览、图片点击灯箱。
- **知识库与画像**：知识库列表与详情；可选画像视图（主题气泡、数据比例）。
- **上传与管道**：上传进度、解析/向量化/画像更新等阶段状态展示。
- **调试**：RAG 检查器、引用得分构成、上下文窗口前后文等（InspectorDrawer 等）。

前后端数据结构约定（SSE 事件、Citation 结构等）见原文档“数据结构定义”部分及前端 `architectureData`、Stream 相关实现。

**实现方案要点：**

- **ThinkingCapsule**：消费 SSE 的 `thought` 事件，将 intent_type、refined_query、sub_queries、target_kbs（含 id、name、score）、routing_method、以及 Dense/Sparse/Visual 是否激活、sparse_keywords 等展示为可读状态或迷你图表，便于用户理解「为何选这些库、用了哪些检索路」。
- **CitationPopover**：根据消息中的 [n] 与 citation 列表匹配，悬停时展示对应条目的 file_name、content 摘要、类型；若 backend 在 debug_info 中提供 context_window，可展示前文/后文或「展开上下文」。
- **图片与音视频**：引用类型为 image 时展示缩略图或 Presigned URL，点击进入灯箱；audio/video 类型可嵌入播放器，video 可展示关键帧与时间戳。
